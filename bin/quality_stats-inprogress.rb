...
@duration = Hitimes::Interval.measure do
  ...
  pkgrecs = recs.select {|rec| rec.packages.include?(@the_package)}
  recs = pkgrecs
  index = 0
  ...
  recs.each do |r|
    briefs += 1 if r['001'].value =~ /ss[ei]b/
    full += 1 if r['001'].value =~ /ss[ej]\d/
  ...
  statsheet = CSV.open('output/pkg_stats.csv', "w") do |csv|
    pkg_stats.each do |r|
      csv << r
